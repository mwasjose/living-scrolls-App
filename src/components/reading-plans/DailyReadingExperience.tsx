  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="rounded-[24px] border border-border bg-gradient-to-br from-accent-soft via-midnight/40 to-midnight/60 p-8 shadow-soft backdrop-blur-xl"
      >
        <div className="flex items-start justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.28em] text-accent">{reading.date}</p>
          ...
          <button
            ...
            className={`rounded-full p-3 transition-all ${
              reading.bookmarked
                ...
                : 'bg-card-soft text-[var(--text-secondary)] hover:bg-surface-soft'
            }`}
          >
      ...
      {activeTab === 'scripture' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="space-y-4 rounded-[24px] border border-border bg-secondary p-8 shadow-soft backdrop-blur-xl">
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-accent-soft px-4 py-2 text-sm font-medium text-accent hover:bg-accent-soft transition-colors"
            >
          ...
          {reading.wordStudy && (
            <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-6 shadow-soft">
              <h3 className="text-sm uppercase tracking-widest text-accent">Hebrew Word Study</h3>
      ...
      {activeTab === 'commentary' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="space-y-3 rounded-[24px] border border-border bg-secondary p-8 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Devotional Reflection</h3>
            <p className="text-lg leading-relaxed text-slate-100">{reading.devotionalCommentary}</p>
          </div>

          {/* Hebraic Context */}
          <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-6 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Hebraic Context</h3>
            <p className="text-slate-300">{reading.hebraicContext}</p>
          </div>

          {/* Messianic Connection */}
          <div className="space-y-3 rounded-[24px] border border-accent bg-accent-soft p-6 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Connection to Yahshuah Messiah</h3>
            <p className="text-slate-100">{reading.messianicConnection}</p>
          </div>

          {/* Life Application */}
          <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-6 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Living This Truth</h3>
            <p className="text-slate-300">{reading.lifeApplication}</p>
          </div>

          {/* Prayer Suggestion */}
          <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-6 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Prayer Thought</h3>
      ...
      {activeTab === 'reflection' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          {/* Reflection Prompt */}
          <div className="rounded-[24px] border border-accent bg-accent-soft p-6 shadow-soft">
            <h3 className="text-sm uppercase tracking-widest text-accent">Reflection Prompt</h3>
            <p className="mt-3 text-lg text-white">{reading.reflectionPrompt}</p>
          </div>

          {/* Journal Entry */}
          <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-8 shadow-soft">
            ...
            <textarea
              ...
              className="min-h-64 w-full bg-card-soft rounded-[12px] border border-border px-4 py-3 text-slate-100 placeholder-slate-500 transition-all focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
            />
            <button
              onClick={handleSaveReflection}
              disabled={saving}
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-accent px-6 py-3 font-semibold text-slate-950 transition-all hover:bg-accent-soft disabled:opacity-60"
            >
          ...
          {/* Additional Notes */}
          <div className="space-y-3 rounded-[24px] border border-border bg-surface-soft p-6 shadow-soft">
            <label className="text-sm uppercase tracking-widest text-accent">Personal Notes</label>
            <textarea
              defaultValue={reading.userNotes}
              placeholder="Add any additional notes, cross-references, or personal insights..."
              className="min-h-40 w-full bg-card-soft rounded-[12px] border border-border px-4 py-3 text-slate-100 placeholder-slate-500 transition-all focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
            />
          </div>
        </motion.div>
      )}
      ...
      <div className="flex justify-between items-center pt-4">
        ...
        <button className="rounded-full bg-accent-soft px-4 py-2 text-sm font-medium text-accent hover:bg-accent-soft transition-colors">
          Mark as Complete
        </button>
        <button className="text-sm text-slate-400 hover:text-white transition-colors">
          Next Day →
